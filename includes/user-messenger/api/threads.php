    ...
    register_rest_route('dm/v1','/threads',[
        ...
        'callback'=>function(){
            ...
            foreach($threads as $thread){
                ...
                $last_msg=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}dm_messages WHERE thread_id=%d ORDER BY created_at DESC LIMIT 1",$tid));
                $data[]=[
                    'id'=>$tid,
                    'other_user'=>['id'=>$other,'name'=>get_the_author_meta('display_name',$other),'avatar'=>$avatar_url],
